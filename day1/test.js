...
function allReplace(str, obj) {
  ...
  // for (const x in obj) {
  //   result = result.replace(new RegExp(x, "g"), obj[x]);
  // }
  // return result;
...
for (const word of input) {
  console.log(word);
  const newWord = allReplace(word.toString(), {
    one: 1,
    two: 2,
    three: 3,
    four: 4,
  ...
  console.log(newWord);
  const number = newWord.split("").filter((char) => !isNaN(parseInt(char)));
  console.log(number);
  const sum = parseInt(number[0] + number[number.length - 1]);

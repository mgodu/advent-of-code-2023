...
function findNumberAdjacentToSymbol(arr) {
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    for (let j = 0; j < symbols.length; j++) {
      ...
      numberMatchDouble.push(numbersMatch);
    }
  }

  numberMatchDouble.forEach((arr) => {
    return removeDuplicates(arr);
  });

  const numberMatchesWithoutDouble = numberMatchDouble.map((arr) =>
    removeDuplicates(arr)
  );

  const filteredNumberMatches = numberMatchesWithoutDouble.filter(
    (arr) => arr.length === 2
  );

  const matches = [];
  filteredNumberMatches.forEach((arr) => {
    matches.push(arr[0].number * arr[1].number);
  });
